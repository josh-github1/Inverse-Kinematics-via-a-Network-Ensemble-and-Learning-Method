# Training the original and best model for 3000 epochs to test against
...
m1 = model1()
m4 = model4()
history1 = m1.fit(X, Y, epochs=3000)
history4 = m4.fit(X, Y, epochs=3000)

plt.plot(history1.history['accuracy'], label="Model 1")
plt.plot(history4.history['accuracy'], label="Model 4")

plt.title('Model Accuracy for original and best networks, 1000 data points 3000 epochs')
plt.ylabel('Accuracy')
plt.xlabel('Epoch')
...
plt.title('Model Loss for original and best networks, 1000 data points 3000 epochs')
plt.ylabel('Loss')
plt.xlabel('Epoch')
plt.legend(loc='upper left', borderaxespad=0.)
plt.show()
